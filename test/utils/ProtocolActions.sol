...
contract ProtocolActions is TestUtils {
    ...
    function addLiq(
        ...
        (uint256 toAdd0, uint256 toAdd1,) = IL1Router(_L1Router).quoteAddLiquidity(
            IDove(_dove).token0(),
            IDove(_dove).token1(), 
            _amount0In,
            _amount1In
    ...
    function removeLiq(
        ...
        (uint256 toRemove0, uint256 toRemove1) = IL1Router(_L1Router).quoteRemoveLiquidity(
            IDove(_dove).token0(),
            IDove(_dove).token1(), 
            liquidity
        );
    ...
    function simpleSwapTokensForTokens(
        ...
        if(_fromToken == IPair(_pair).token0()) {
            uint256 amountOutMin = IL2Router(_L2Router).getAmountOut(
                amountIn,
                IPair(_pair).token0(),
                IPair(_pair).token1()
            );
